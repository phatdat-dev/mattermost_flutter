...
class _ChannelNotificationPreferencesScreenState extends State<ChannelNotificationPreferencesScreen> {
  ...
  bool _ignoreChannelMentions = false;
  String _muteChannel = 'off';
  ...
  Future<void> _loadNotificationPreferences() async {
    ...
    try {
      ...
      setState(() {
        ...
        _markUnread = notifyProps['mark_unread'] ?? 'all';
        _ignoreChannelMentions = notifyProps['ignore_channel_mentions'] == 'on';
        _muteChannel = notifyProps['mute_channel'] ?? 'off';
      });
    } catch (e) {
      setState(() {
  ...
  Future<void> _saveNotificationPreferences() async {
    ...
    try {
      final notifyProps = {
        ...
        'mark_unread': _markUnread,
        'ignore_channel_mentions': _ignoreChannelMentions ? 'on' : 'off',
                ...
                children: [
                  ...
                  Card(
                    child: SwitchListTile(
                      title: const Text('Ignore mentions for @channel, @here and @all'),
                      subtitle: const Text('When enabled, @channel, @here and @all will not trigger mentions for you'),
                      value: _ignoreChannelMentions,
                      onChanged: (value) => setState(() => _ignoreChannelMentions = value),
                    ),
                  ),
                  const SizedBox(height: 16),
                  ...
                  Card(
                    child: SwitchListTile(
                      ...
                      subtitle: const Text('When enabled, this channel will not appear in unread channels and notifications will be disabled'),
                      value: _muteChannel == 'on',
                      onChanged: (value) => setState(() => _muteChannel = value ? 'on' : 'off'),
                    ),
                  ),
