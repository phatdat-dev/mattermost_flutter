...
class MSystemApi {
  ...
  /// Get system status
  Future<MSystemStatus> getSystemStatus() async {
    try {
      final response = await _dio.get('/api/v4/system/status');
      return MSystemStatus.fromJson(response.data);
    } catch (e) {
      rethrow;
    }
  }
  ...
  /// Get analytics
  Future<Map<String, dynamic>> getAnalytics({String? teamId}) async {
    try {
