...
class MWebhooksApi {
  ...
  Future<MIncomingWebhook> createIncomingWebhook(MCreateIncomingWebhookRequest request) async {
    try {
      final response = await _dio.post('/api/v4/hooks/incoming', data: request.toJson());
  ...
  Future<List<MIncomingWebhook>> getIncomingWebhooks({int page = 0, int perPage = 60, String? teamId}) async {
    try {
      ...
      final response = await _dio.get('/api/v4/hooks/incoming', queryParameters: queryParams);
  ...
  Future<MIncomingWebhook> updateIncomingWebhook(String hookId, MUpdateIncomingWebhookRequest request) async {
    try {
      final response = await _dio.put('/api/v4/hooks/incoming/$hookId', data: request.toJson());
      return MIncomingWebhook.fromJson(response.data);
    } catch (e) {
      rethrow;
    }
  ...
  Future<MOutgoingWebhook> createOutgoingWebhook(MCreateOutgoingWebhookRequest request) async {
    try {
      final response = await _dio.post('/api/v4/hooks/outgoing', data: request.toJson());
  ...
  Future<List<MOutgoingWebhook>> getOutgoingWebhooks({int page = 0, int perPage = 60, String? teamId, String? channelId}) async {
    try {
      final Map<String, dynamic> queryParams = {'page': page.toString(), 'per_page': perPage.toString()};
      ...
      final response = await _dio.get('/api/v4/hooks/outgoing', queryParameters: queryParams);
  ...
  Future<MOutgoingWebhook> updateOutgoingWebhook(String hookId, MUpdateOutgoingWebhookRequest request) async {
    try {
      final response = await _dio.put('/api/v4/hooks/outgoing/$hookId', data: request.toJson());
      return MOutgoingWebhook.fromJson(response.data);
    } catch (e) {
      rethrow;
    }
  ...
  Future<MOutgoingWebhook> regenerateOutgoingWebhookToken(String hookId) async {
    try {
      final response = await _dio.post('/api/v4/hooks/outgoing/$hookId/regen_token');
      return MOutgoingWebhook.fromJson(response.data);
    } catch (e) {
      rethrow;
    }
